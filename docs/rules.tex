...
\begin{definition}[Winning]
  ...
  \begin{enumerate}[nosep, label=(\roman*)]
    ...
    \item $p \notin \Winner$ (a player can win at most once).
  \end{enumerate}
  The game ends when $n-1$ players have won (the last remaining player loses).
\end{definition}

\begin{rule}[Winner leaves rotation]
  Once a player joins $\Winner$, they are removed from the active turn
  rotation. The \texttt{nextPlayerIndex} function skips over all players
  in $\Winner$ when advancing the turn. Winners take no further actions.
\end{rule}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Turn advance:} $i_{\text{curr}} \leftarrow
        \func{nextActive}(i_{\text{curr}} + \Dir)$, where
        $\func{nextActive}$ skips over any $i$ such that
        $\Players[i].id \in \Winner$.
\end{enumerate}
...
\begin{invariant}[Winner uniqueness]
  No player appears in $\Winner$ more than once. Formally,
  $|\Winner|$ equals the number of distinct player IDs in $\Winner$.
\end{invariant}
