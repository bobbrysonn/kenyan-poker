...
function botAction(state: GameState): PlayerAction {
	...
	if (state.bomb) {
		...
		if (hasAce && Math.random() < 0.6) {
			const aceIdx = hand.findIndex(
				(c) => c.kind === "regular" && c.rank === "A",
			);
			return {
				kind: "bomb_response",
				action: { kind: "counter", cardIndex: aceIdx },
			};
		}
		...
		if (hasBomb && Math.random() < 0.3) {
			...
			return {
				kind: "bomb_response",
				action: { kind: "counter", cardIndex: bombIdx },
			};
		}
		return { kind: "bomb_response", action: "pick" };
	}
...
while (!state.gameOver && turnCount < 500) {
	...
	if (player.cardless) {
		...
		state = r.newState;
		console.log(`[${turnCount}] ${player.name} 🔄 cardless → auto-draws`);
		continue;
	}
